...
# STEP 1 - Diagnostic
$diagScript = '(function() { var out = []; if (window.monaco) { out.push("Monaco found"); var models = window.monaco.editor.getModels(); out.push("Models: " + models.length); models.forEach(function(m, i) { out.push("Model " + i + ": " + m.uri.toString().substring(0, 80)); }); var editors = window.monaco.editor.getEditors(); out.push("Editors: " + editors.length); } else { out.push("Monaco NOT found"); } var pineEditors = document.querySelectorAll("[class*=\"pine\"], [class*=\"Pine\"], [class*=\"editor\"], [class*=\"Editor\"]"); out.push("Pine/Editor DOM elements: " + pineEditors.length); if (window.CodeMirror) out.push("CodeMirror found"); return out.join(" | "); })()'

$req1 = '{"id":1,"method":"Runtime.evaluate","params":{"expression":' + ($diagScript | ConvertTo-Json) + ',"returnByValue":true}}'
Send-CDP $req1
Start-Sleep -Milliseconds 2000
$resp1 = Recv-CDP
Write-Output "STEP1_RESULT: $resp1"
...
$injectionJs = @"
(function() {
  var pineCode = $($pineCode | ConvertTo-Json);
  if (window.monaco) {
    var models = window.monaco.editor.getModels();
    var pineModel = null;
    models.forEach(function(m) {
      var uri = m.uri.toString();
      if (uri.includes('pine') || uri.includes('script') || uri.includes('Pine')) { pineModel = m; }
    });
    if (!pineModel && models.length > 0) pineModel = models[0];
    if (pineModel) {
      var range = pineModel.getFullModelRange();
      var editors = window.monaco.editor.getEditors();
      if (editors.length > 0) {
        editors[0].executeEdits('inject', [{ range: range, text: pineCode }]);
        return 'SUCCESS: Injected ' + pineCode.length + ' chars';
      } else {
        pineModel.setValue(pineCode);
        return 'SUCCESS: Set value on model';
      }
    } else { return 'ERROR: No Monaco model found'; }
  }
  return 'ERROR: Monaco not available';
})()
"@

$req2 = '{"id":2,"method":"Runtime.evaluate","params":{"expression":' + ($injectionJs | ConvertTo-Json) + ',"returnByValue":true}}'
Send-CDP $req2
Start-Sleep -Milliseconds 3000
$resp2 = Recv-CDP
Write-Output "STEP2_RESULT: $resp2"

# STEP 3 - Save
$saveJs = '(function() { var btns = document.querySelectorAll("button"); var saveBtn = null; for (var b of btns) { var txt = (b.textContent || b.innerText || "").trim().toLowerCase(); var title = (b.title || "").toLowerCase(); if (txt === "save" || txt === "add to chart" || title.includes("save") || title.includes("add to chart")) { saveBtn = b; break; } } if (saveBtn) { saveBtn.click(); return "Clicked: " + saveBtn.textContent; } document.dispatchEvent(new KeyboardEvent("keydown", {key: "s", ctrlKey: true, bubbles: true})); return "Sent Ctrl+S"; })()'

$req3 = '{"id":3,"method":"Runtime.evaluate","params":{"expression":' + ($saveJs | ConvertTo-Json) + ',"returnByValue":true}}'
Send-CDP $req3
Start-Sleep -Milliseconds 2000
$resp3 = Recv-CDP
Write-Output "STEP3_RESULT: $resp3"
